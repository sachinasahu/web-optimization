...
function getNoun(y) {
  switch(y) {
    ...
    case "jewelry":
      ...
      return jewelry;
    case "places":
      var places = ["swamp", "graveyard", "cemetery", "park", "Productioning", "house", "river", "ocean", "sea", "field", "forest", "woods", "neighborhood",
...
var resizePizzas = function(size) {
  ...
  var pizzaSlider = document.querySelector("#pizzaSize");  //Moved "document.querySelector("#pizzaSize").innerHTML" to the variable named pizzaSlider practicing D.R.Y.
  function changeSliderLabel(size) {
    switch(size) {
      case "1":
        pizzaSlider.innerHTML = "Small";
        return;
      case "2":
        pizzaSlider.innerHTML = "Medium";
        return;
      case "3":
        pizzaSlider.innerHTML = "Large";
        return;
      default:
  ...
  function changePizzaSizes(size) {
    ...
    var randomPizzas = document.getElementsByClassName("randomPizzaContainer"); //changed querySelectorAll to getElementsByClassName for faster performance.
...
for (var i = 2; i < 45; i++) { //changed number of random pizzas generated from 100 to 50.  Still giving customers plenty of pizza choices but cutting down on page expenses.
...
function updatePositions() {
  ...
  //created new variables to hold lookups outside the loop, for faster performance times.  Following tutorial written by mcs found here:
  //https://discussions.udacity.com/t/project4-how-do-i-optimize-the-pizza-slider-resize-for-loop/37297

  var items = document.getElementsByClassName('mover'); //changed querySelectorAll to getElementsByClassName for faster performance.
  var top = document.body.scrollTop / 1250;
  var constArray = [];
  var cachedLength = items.length;

  for(i=0; i < 3; i++){  //changed test expression for less unique pizzas generating but still creating the same effect.
    constArray.push(Math.sin((top)+ i));
  }

  for (var i = 0; i < cachedLength; i++) {
    var phase = constArray[i % 3]; //Changed value after Modelo to adhere with for loop generating the pizzas above.
  ...
  window.requestAnimationFrame.bind(updatePositions);
  }
}
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  for (var i = 0; i < 30; i++) {  //changed scrolling pizza numbers from 200 to 30, creating same effect without being so expensive.
